import Link from "next/link"
import { Facebook, Instagram, Linkedin, Mail, MapPin, Phone } from 'lucide-react'

export default function Footer() {
  return (
    <footer className="bg-black text-white">
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          <div>
            <h3 className="text-xl font-bold mb-4">Impact IT</h3>
            <p className="text-gray-400 mb-4">Profesjonelt IT-utstyr på utleie for bedrifter og privatpersoner.</p>
            <div className="flex space-x-4">
              <Link href="https://facebook.com" className="text-gray-400 hover:text-white">
                <Facebook className="h-5 w-5" />
                <span className="sr-only">Facebook</span>
              </Link>
              <Link href="https://instagram.com" className="text-gray-400 hover:text-white">
                <Instagram className="h-5 w-5" />
                <span className="sr-only">Instagram</span>
              </Link>
              <Link href="https://linkedin.com" className="text-gray-400 hover:text-white">
                <Linkedin className="h-5 w-5" />
                <span className="sr-only">LinkedIn</span>
              </Link>
            </div>
          </div>

          <div>
            <h3 className="text-lg font-semibold mb-4">Snarveier</h3>
            <ul className="space-y-2">
              <li>
                <Link href="/" className="text-gray-400 hover:text-white">
                  Hjem
                </Link>
              </li>
              <li>
                <Link href="/utstyr" className="text-gray-400 hover:text-white">
                  Produkter
                </Link>
              </li>
              <li>
                <Link href="/ansatte" className="text-gray-400 hover:text-white">
                  Ansatte
                </Link>
              </li>
              <li>
                <Link href="/om-oss" className="text-gray-400 hover:text-white">
                  Om oss
                </Link>
              </li>
              <li>
                <Link href="/kontakt" className="text-gray-400 hover:text-white">
                  Kontakt
                </Link>
              </li>
            </ul>
          </div>

          <div>
            <h3 className="text-lg font-semibold mb-4">Kontakt oss</h3>
            <ul className="space-y-3">
              <li className="flex items-start">
                <MapPin className="h-5 w-5 mr-2 text-gray-400 mt-0.5" />
                <span className="text-gray-400">
                  Teknologiveien 22
                  <br />
                  0371 Oslo
                </span>
              </li>
              <li className="flex items-center">
                <Phone className="h-5 w-5 mr-2 text-gray-400" />
                <a href="tel:+4712345678" className="text-gray-400 hover:text-white">
                  +47 12 34 56 78
                </a>
              </li>
              <li className="flex items-center">
                <Mail className="h-5 w-5 mr-2 text-gray-400" />
                <a href="mailto:kontakt@impactit.no" className="text-gray-400 hover:text-white">
                  kontakt@impactit.no
                </a>
              </li>
            </ul>
          </div>

          <div>
            <h3 className="text-lg font-semibold mb-4">Åpningstider</h3>
            <ul className="space-y-2 text-gray-400">
              <li className="flex justify-between">
                <span>Mandag - Fredag:</span>
                <span>08:00 - 16:00</span>
              </li>
              <li className="flex justify-between">
                <span>Lørdag:</span>
                <span>Stengt</span>
              </li>
              <li className="flex justify-between">
                <span>Søndag:</span>
                <span>Stengt</span>
              </li>
            </ul>
          </div>
        </div>

        <div className="border-t border-gray-800 mt-12 pt-8 text-center text-gray-500">
          <p>&copy; {new Date().getFullYear()} Impact IT. Alle rettigheter reservert.</p>
        </div>
      </div>
    </footer>
  )
}
